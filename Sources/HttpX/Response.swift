...
public class Response: CustomStringConvertible, IteratorProtocol, Sequence, AsyncIteratorProtocol, AsyncSequence {
    ...
    /// The History of the response, will be generated when auth and redirect.
    public var history: [Response] { historyInternal }

    /// When not following redirect, the redirected request will be stored here.
    public var nextRequest: URLRequest? { nextRequestInternal }
    ...
    public func makeIterator() -> Response {
        if mode == .idle {
            mode = .sync
        }
        if mode != .sync {
    ...
    public func makeAsyncIterator() -> Response {
        if mode == .idle {
            mode = .async
        }
        if mode != .async {
    ...
    internal var nextRequestInternal: URLRequest?
